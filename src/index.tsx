...
import { MenuItemUser } from "./ctxMenu";

const { getByStoreName, getByProps } = webpack;
const { ContextMenuTypes } = types
const { ContextMenu: { MenuGroup } } = components

const inject = new Injector()

export function start(): void {
  inject.utils.addMenuItem<{user: MsgAuthor}>(ContextMenuTypes.UserContext, (data, menu) => {
    const userID = data.user.id
    const isCutie = cfg.get("goodUsers").includes(userID)
    const isMeanie = cfg.get("badUsers").includes(userID)

    const children = isCutie == isMeanie ? [
      MenuItemUser({add: true, good: true, userID}),
      MenuItemUser({add: true, good: false, userID}),
    ] : [
      MenuItemUser({add: true, good: !isCutie, userID}),
      MenuItemUser({add: false, good: isCutie, userID}),
    ]

    return <MenuGroup children={children} />
  })
}
